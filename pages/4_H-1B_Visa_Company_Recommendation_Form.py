    ...
    if submit_button:
        ...
        if filtered_df.empty:
            ...
            if len(valid_weights) == 0:
                ...
                result_df.rename(columns={'FULL_WORKSITE_STATE': 'FULL_WORKSITE_STATE_LIST'}, inplace=True)
                result_df['OTHER_WORKSITE_STATE'] = result_df['FULL_WORKSITE_STATE_LIST'].apply(lambda x: x[1:] if len(x) > 1 else [])
                ...
                result_df.rename(columns={'SOC_TITLE': 'SOC_TITLE_LIST'}, inplace=True)
                result_df['OTHER_SOC_TITLES'] = result_df['SOC_TITLE_LIST'].apply(lambda x: x[1:] if len(x) > 1 else [])
                ...
                # Display only unique outputs
                result_df.drop_duplicates(subset=['EMPLOYER_NAME_CLEAN'], keep='first', inplace=True)
                result_df = result_df[['EMPLOYER_NAME', 'SOC_TITLE_LIST', 'FULL_WORKSITE_STATE_LIST', 'PREVAILING_WAGE_ANNUAL', 
                                    'EMPLOYEE_COUNT_CATEGORY', 'COMPANY_AGE_CATEGORY', 'COMPANY_LINK', 
                                    'JOB_COUNT', 'OTHER_WORKSITE_STATE', 'OTHER_SOC_TITLES']]
